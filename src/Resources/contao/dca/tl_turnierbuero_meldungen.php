	...
	(
		...
		(
			...
			(
				...
				'button_callback'     => array('tl_turnierbuero_meldungen', 'toggleIcon')
			),
			'show' => array
			(
...
class tl_turnierbuero_meldungen extends Backend
{
	...
	public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_turnierbuero_meldungen::published', 'alexf'))
		{
			return '';
		}
	...
	public function toggleVisibility($intId, $blnPublished)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_turnierbuero_meldungen::published', 'alexf'))
		{
			$this->log('Kein Zugriffsrecht für Aktivierung Datensatz ID "'.$intId.'"', 'tl_turnierbuero_meldungen toggleVisibility', TL_ERROR);
		...
		$this->createInitialVersion('tl_turnierbuero_meldungen', $intId);
		
		// Trigger the save_callback
		if (is_array($GLOBALS['TL_DCA']['tl_turnierbuero_meldungen']['fields']['published']['save_callback']))
		{
			foreach ($GLOBALS['TL_DCA']['tl_turnierbuero_meldungen']['fields']['published']['save_callback'] as $callback)
			{
		...
		// Update the database
		$this->Database->prepare("UPDATE tl_turnierbuero_meldungen SET tstamp=". time() .", published='" . ($blnPublished ? '' : '1') . "' WHERE id=?")
					   ->execute($intId);
		$this->createNewVersion('tl_turnierbuero_meldungen', $intId);
	}
